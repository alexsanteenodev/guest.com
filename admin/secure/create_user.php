...
<form action="<?= $_SERVER['PHP_SELF']?>" method="post">
	<div>
		...
		<input id="User" type="text" name="user" value="<?= $user?>" style="width:40em"/>
	</div>
	<div>
		<label for="String">Пароль</label>
		<input id="String" type="text" name="string" value="<?= $string?>" style="width:40em"/>
	</div>
	<div>
		<label for="Salt">Соль</label>
		<input id="Salt" type="text" name="salt" value="<?= $salt?>"  style="width:40em"/>
	</div>	
	<div>
		<label for="IterationCount">Число иттераций</label>
		<input id="IterationCount" type="text" name="n" value="<?= $iterationCount?>"  style="width:4em"/>
	</div>		
	<div>
